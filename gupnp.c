...
zend_function_entry gupnp_functions[] = {
	PHP_FE(confirm_gupnp_compiled,	NULL)		/* For testing, remove later. */
...
PHP_FUNCTION(confirm_gupnp_compiled)
{
	char *arg = NULL;
	int arg_len, len;
	char *strg;
	...
	len = spprintf(&strg, 0, "Congratulations! You have successfully modified ext/%.78s/config.m4. Module %.78s is now compiled into PHP.", "gupnp", arg);
